...
def travoltas_busiest_years
  ...
  execute(<<-SQL)
  SELECT
    movie_count.yr, movie_count.count
  FROM
    (
      SELECT
        COUNT(movies.title), yr
      FROM
        movies
      JOIN
        castings ON movies.id = castings.movie_id
...
def andrews_films_and_leads
  ...
  JOIN
    castings ON her_movies.id = castings.movie_id
  JOIN
    actors ON castings.actor_id = actors.id
  WHERE
    castings.ord = 1
...
def colleagues_of_garfunkel
  ...
  JOIN
    castings ON garfunkel_films.id = castings.movie_id
  JOIN
    actors ON castings.actor_id = actors.id
  WHERE
    actors.name != 'Art Garfunkel'
